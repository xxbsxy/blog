...
const App = memo(() => {
  ...
  return <div className={`title ${isActive ? "active" : ""} `}>App</div>;
});
...
const App = memo(() => {
  ...
  return <div className={classList.join(" ")}>App</div>;
});

export default App;
```

#### 2.1.3 写法三使用 classnames 库
...
const App = memo(() => {
  const [isActive, setIsActive] = useState(true);
  // isActive为true时 动态添加active类名
  return <div className={classNames("total", { active: isActive })}>App</div>;
});
...
const App = memo(() => {
  return <div style={{ color: "red", fontSize: "28px" }}>App</div>;
});
...
const App = memo(() => {
  ...
  return (
    <div className="app">{isShow ? <button>show</button> : <h2>show</h2>}</div>
  );
});
...
const App = memo(() => {
  ...
  return <div className="app"> {obj && obj.name} </div>;
});
...
const App = memo(() => {
  ...
  // 例如obj是服务器请求的数据 刚开始可能为空
  return <div className="app"> {obj?.name} </div>;
});
...
当一个表单绑定了 value 属性并且值来源于 state，并且只能通过使用 setState 来更新，使 React 的 state 成为“唯一数据源”，被 React 以这种方式控制取值的表单输入元素就叫做“受控组件”。
...
非受控组件的表单数据将交由 DOM 节点来处理，使用 ref 来获取输入的值
...
需要把.css/.less/.scss 文件编写成.module.css/.module.less/.module.scss
...
2. 在 vscode 安装 vscode-styled-components 插件
...
2. **新建 store/actionCreatore.js 动态生成 action**
...
3. **新建 store/reducer.js 存放 reducer 函数**
...
3. 新建 store/features/counter.js
...
使用 createAsyncThunk 和 extraReducers 进行异步操作
...
const homeSlice = createSlice({
  ...
  extraReducers(builder) {
    ...
    // fulfilled状态
    builder.addCase(
      fetchChangeBannersAction.fulfilled,
      (state, { payload }) => {
...
使用 createAsyncThunk 但不使用 extraReducers
